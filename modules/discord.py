...
file_with_build_num = (
    ...
    + re.compile("assets/+([a-z0-9]+)\\.js").findall(res)[-2]
    + ".js"
)
...
class Discord:
    async def get_headers(self, payload):
        ...
        __dcfduid = [item for item in cookies if item["name"] == "__dcfduid"][0][
            "value"
        ]
        __sdcfduid = [item for item in cookies if item["name"] == "__sdcfduid"][0][
            "value"
        ]
        ...
        super_props = {
            ...
            "client_version": self.browser.browser.version,
            "os_version": str(platform.version()),
            "os_arch": "x64" if platform.machine().endswith("64") else "x86",
            "system_locale": self.browser.faker.locale,
            "client_build_number": DISCORD_BUILD_NUM,
    ...
    async def humanize_token(self):
        ...
        self.page.on(
            "filechooser", lambda file_chooser: file_chooser.set_files(temp_file.name)
        )
        upload_avatar_button = self.page.locator(
            '[class *= "buttonsContainer-12kYno"]'
        ).locator('[class *= "lookFilled-yCfaCM "]')
    ...
    async def join_server(self):
        await self.page.goto("https://discord.com/channels/@me")
        await self.page.wait_for_timeout(1000)
        create_join_button = self.page.locator(
            '[data-list-item-id *= "create-join-button"]'
        )
        ...
        await self.page.wait_for_timeout(1000)
        await self.page.type("[placeholder='https://discord.gg/dev']", self.invite_link)
    ...
    async def is_locked(self):
        token_check = await self.page.request.get(
    ...
    async def set_email(self, email):
        try:
            ...
            await settings_button.click()
            await self.page.wait_for_timeout(500)
            settings_button = self.page.locator('[class *= "fieldButton-14lHvK"]').nth(
                1
            )
            await settings_button.click()
    ...
    async def confirm_email(self):
        ...
        if is_locked:
            self.logger.error(
                f"Token {self.token} got locked whilst verifying the Email!"
            )
            await self.close()
            return
        self.log_output()
